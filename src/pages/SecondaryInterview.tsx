...
const SecondaryInterview = () => {
  ...
  const { data: jobs = [], isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/jobs?stage=Secondary%20Interview`, {
        headers: {
  ...
  const handleViewDetails = (jobId: string) => {
    navigate(`/dashboard/jobs/${jobId}?stage=Secondary%20Interview`);
  };
  ...
  useEffect(() => {
    if (selectedTab === 'upcoming' || selectedTab === 'past') {
      const fetchInterviews = async () => {
        ...
        try {
          const token = localStorage.getItem('token');
          const stage = 'Secondary Interview';
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-blue-50 p-0">
      ...
      <div className="max-w-6xl mx-auto pt-8 pb-4 px-4">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-1">Secondary Interview Jobs</h1>
            <p className="text-gray-600 text-base">Advanced technical and behavioral assessments</p>
          </div>
        </div>
        {/* Stats Cards */}
